...
public class GenerateNewKeyPairDialog extends JDialog {
  ...
  private JSpinner publicExponentSpinner;
  ...
  public GenerateNewKeyPairDialog() {
    ...
    publicExponentSpinner.setValue(65537);
    publicExponentSpinner.setEditor(new JSpinner.NumberEditor(publicExponentSpinner, "#"));
          ...
          protected Void doInBackground () throws Exception {
            ...
            while (!isCancelled() && state < 3) {
              ...
              if (state == 0) {
                ...
                try {
                  keyPair = RSAKeyPair.generateKeyPair(prime1, prime2, BigNumber.fromInt((int) publicExponentSpinner.getValue()));
                }
    ...
    keyLengthSpinner.addChangeListener(new ChangeListener() {
      ...
      public void stateChanged(ChangeEvent e) {
        try {
          maxBytesLabel.setText(String.format("%d", Message.getLengthLimit("SHA-1", (int) keyLengthSpinner.getValue() / 8)));
        }
        catch (NoSuchAlgorithmException ex) {
          JOptionPane.showMessageDialog(contentPane, "Fatal error - algorithm not found", "Generate new key pair", JOptionPane.ERROR_MESSAGE);
          System.exit(-1);
        }
      }
    });
